...
class NodeAPITest(APITestCase):
    ...
    def test_node_get(self):
        response = self.client.get("/node/")
        self.assertNotEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get(f"/node/get/{uuid.uuid4()}/")
        ...
        response = self.client.get(f"/node/get/{node['id']}/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, node)

    def test_node_update(self):
        invalid_data = {"bob": "peter"}
        response = self.client.put("/node/update/", invalid_data, format="json")

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data, NODE_ID_REQUIRED_MSG)

        response = self.client.put(
            "/node/update/", {"id": uuid.uuid4(), "name": "test"}, format="json"
        )
        ...
        response = self.client.get(f"/node/get/{node1_id}/")
    ...
    def test_node_delete(self):
        response = self.client.delete(f"/node/delete/{uuid.uuid4()}/")
        ...
        response = self.client.delete(f"/node/delete/{response.data['id']}/")

        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.delete(f"/node/delete/{node_id}/")
